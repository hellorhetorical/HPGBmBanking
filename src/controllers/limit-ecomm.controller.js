(() => {
  ...
  angular.module("hpgbmbanking").controller("LimitEcommController", [
      "$location",
      function ($location) {
      const vm = this;
      vm.title = "Update EComm Limit";
      vm.subtitle = "Set a new EComm spending limit.";
      vm.accounts = getMockList(MOCK_DATA.LIMIT_ECOMM_ACCOUNTS);
      ...
      vm.cardPinError = "";
      vm.statusMessage = "";
      vm.standardLimit = DEFAULT_LIMIT_STANDARD_LIMITS.ecomm || "";
      ...
      vm.continueFromUpdate = function () {
        if (!vm.form.ecommLimit) {
          vm.statusMessage = "Enter the new EComm limit.";
          return;
        }
        const enteredLimit = Number.parseFloat(
          vm.form.ecommLimit.toString().replace(/,/g, "")
        );
        if (!Number.isFinite(enteredLimit)) {
          vm.statusMessage = "Enter a valid EComm limit.";
          return;
        }
        if (enteredLimit < 0) {
          vm.statusMessage = "EComm limit cannot be below 0.";
          return;
        }
        if (standardLimitValue && enteredLimit > standardLimitValue) {
          vm.statusMessage = "EComm limit cannot exceed the standard limit.";
          return;
        }
        vm.statusMessage = "";
        vm.step = "authorize";
