(() => {
  ...
  angular.module("hpgbmbanking").controller("QuickPayStatusController", [
      "$location",
      "$rootScope",
      function ($location, $rootScope) {
        const vm = this;
        const status = $rootScope.quickPayStatus;
        if (!status) {
          $location.path("/quickpay");
          return;
        }
        vm.isSuccess = status.status === "success";
        vm.summary = status;
        vm.statusMessage = vm.isSuccess
          ? `Transaction with reference ID ${vm.summary.reference} processed successfully.`
          : `Transaction with reference ID ${vm.summary.reference} failed. Please try again.`;

        vm.ok = function () {
          $rootScope.quickPayDraft = null;
          $rootScope.quickPayStatus = null;
          $location.path("/home");
        };
      },
    ]);
