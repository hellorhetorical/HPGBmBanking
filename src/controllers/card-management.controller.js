(() => {
  ...
  angular.module("hpgbmbanking").controller("CardManagementController", [
      "$location",
      function ($location) {
      const vm = this;
      vm.title = "Card Management";
      vm.subtitle = "Enable or disable debit card usage channels.";
      ...
      const cardManagementCards =
        getMockObject(MOCK_DATA.CARD_MANAGEMENT_CARDS_BY_ACCOUNT);
      vm.cardsByAccount =
        Object.keys(cardManagementCards).length > 0
          ? cardManagementCards
          : {};
      vm.form = {
