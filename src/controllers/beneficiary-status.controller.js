(() => {
  ...
  angular.module("hpgbmbanking").controller("BeneficiaryStatusController", [
      "$location",
      "$rootScope",
      function ($location, $rootScope) {
        const vm = this;
        vm.title = "Status";
        vm.subtitle = "Payee added successfully.";
        vm.status = $rootScope.beneficiaryStatus;
        if (!vm.status) {
          $location.path("/beneficiaries");
          return;
        }
        vm.ok = function () {
          const initials = (vm.status.name || "")
            .split(" ")
            .map((part) => part[0])
            .join("")
            .slice(0, 2)
            .toUpperCase();
          const bankLabel =
            vm.status.type === "other"
              ? `Other Bank - ${vm.status.transactionType || "IMPS"}`
              : "Same Bank";
          const entry = {
            initials: initials || "NA",
            account: vm.status.accountNumber,
            name: vm.status.nickname,
            bank: bankLabel,
          };
          $rootScope.beneficiariesList = [entry].concat(
            Array.isArray($rootScope.beneficiariesList) ? $rootScope.beneficiariesList : []
          );
          $rootScope.pendingPayee = null;
          $rootScope.beneficiaryStatus = null;
          $location.path("/beneficiaries");
        };
      },
    ]);
