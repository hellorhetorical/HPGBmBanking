(() => {
  ...
  angular.module("hpgbmbanking").controller("LimitContactlessController", [
      "$location",
      function ($location) {
      const vm = this;
      vm.title = "Update Contactless Limit";
      vm.subtitle = "Set a new contactless spending limit.";
      vm.accounts = getMockList(MOCK_DATA.LIMIT_CONTACTLESS_ACCOUNTS);
      ...
      vm.cardPinError = "";
      vm.statusMessage = "";
      vm.standardLimit = DEFAULT_LIMIT_STANDARD_LIMITS.contactless || "";
      ...
      vm.continueFromUpdate = function () {
        if (!vm.form.contactlessLimit) {
          vm.statusMessage = "Enter the new contactless limit.";
          return;
        }
        const enteredLimit = Number.parseFloat(
          vm.form.contactlessLimit.toString().replace(/,/g, "")
        );
        if (!Number.isFinite(enteredLimit)) {
          vm.statusMessage = "Enter a valid contactless limit.";
          return;
        }
        if (enteredLimit < 0) {
          vm.statusMessage = "Contactless limit cannot be below 0.";
          return;
        }
        if (standardLimitValue && enteredLimit > standardLimitValue) {
          vm.statusMessage = "Contactless limit cannot exceed the standard limit.";
          return;
        }
        vm.statusMessage = "";
        vm.step = "authorize";
