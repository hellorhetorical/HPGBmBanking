(() => {
  ...
  angular.module("hpgbmbanking").controller("RdController", function () {
      const vm = this;
      vm.title = "RD Calculator";
      vm.subtitle = "Estimate returns for recurring deposits.";
      vm.form = {
        monthly: "",
        rate: "",
      ...
      vm.calculate = function () {
        const monthlyDeposit = parseFloat(vm.form.monthly);
        const rateAnnual = parseFloat(vm.form.rate);
        const months = parseInt(vm.form.months, 10);
        if (!monthlyDeposit || !rateAnnual || !months || months <= 0) {
          vm.result = null;
          return;
        }
        ...
        const interestEarned = fv - totalDeposited;
