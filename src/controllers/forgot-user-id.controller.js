(() => {
  ...
  angular.module("hpgbmbanking").controller("ForgotUserIdController", [
      "$location",
      "$routeParams",
      function ($location, $routeParams) {
        const vm = this;
        vm.flow = $routeParams.flow || "login";
        vm.userId = "";
        vm.error = "";
        ...
        vm.continue = function () {
          const value = sanitizeUserId(vm.userId);
          vm.userId = value;
          if (!value) {
          ...
          vm.error = "";
          $location.path("/forgot-otp").search({
            flow: vm.flow,
            userId: value,
          });
        };
