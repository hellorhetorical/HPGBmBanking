(() => {
  ...
  angular.module("hpgbmbanking").controller("TransferOtpController", [
      "$location",
      "$rootScope",
      "$interval",
      "$scope",
      function ($location, $rootScope, $interval, $scope) {
        ...
        const draft = $rootScope.transferDraft;
        if (!draft) {
          $location.path("/initiatetransfer");
          return;
        }
        const userLabel = defaultUser.name || defaultUser.userId || "";
        vm.summary = {
          from: userLabel
            ? `${draft.fromAccount} (INR) - ${userLabel}`
            : `${draft.fromAccount} (INR)`,
          to: draft.toAccount,
        ...
        vm.onOtpKeydown = function (index, $event) {
          const key = $event && $event.key ? $event.key : "";
          const inputs = getOtpInputs($event ? $event.target : null);
          if (!inputs.length) {
            return;
          }
          vm.activeOtpIndex = index;
          if (key === "Backspace") {
            if (!vm.otpDigits[index] && index > 0) {
            }
            return;
          }
        };
        ...
        vm.verify = function () {
          ...
          const now = new Date();
          $rootScope.transferSummary = {
            from: vm.summary.from,
            to: vm.summary.to,
            amount: vm.summary.amount,
            remarks: vm.summary.remarks,
            paymentOption: vm.summary.paymentOption,
            scheduleDate: vm.summary.scheduleDate,
            ...
            flowType: draft.flowType || "own",
          };
          $location
            .path("/transfer-status")
            .search({ status: success ? "success" : "failure" });
        };
