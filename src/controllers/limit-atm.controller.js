(() => {
  ...
  angular.module("hpgbmbanking").controller("LimitAtmController", [
      "$location",
      function ($location) {
      const vm = this;
      vm.title = "Update ATM Limit";
      vm.subtitle = "Set a new ATM withdrawal limit.";
      vm.accounts = getMockList(MOCK_DATA.LIMIT_ATM_ACCOUNTS);
      ...
      vm.cardPinError = "";
      vm.statusMessage = "";
      vm.standardLimit = DEFAULT_LIMIT_STANDARD_LIMITS.atm || "";
      ...
      vm.continueFromUpdate = function () {
        if (!vm.form.atmLimit) {
          vm.statusMessage = "Enter the new ATM limit.";
          return;
        }
        const enteredLimit = Number.parseFloat(
          vm.form.atmLimit.toString().replace(/,/g, "")
        );
        if (!Number.isFinite(enteredLimit)) {
          vm.statusMessage = "Enter a valid ATM limit.";
          return;
        }
        if (enteredLimit < 0) {
          vm.statusMessage = "ATM limit cannot be below 0.";
          return;
        }
        if (standardLimitValue && enteredLimit > standardLimitValue) {
          vm.statusMessage = "ATM limit cannot exceed the standard limit.";
          return;
        }
        vm.statusMessage = "";
        vm.step = "authorize";
