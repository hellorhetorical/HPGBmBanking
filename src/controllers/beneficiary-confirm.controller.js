(() => {
  ...
  angular.module("hpgbmbanking").controller("BeneficiaryConfirmController", [
      "$location",
      "$rootScope",
      function ($location, $rootScope) {
        const vm = this;
        vm.title = "Confirm And Authorize";
        vm.subtitle = "Verify payee details and authorize.";
        vm.payee = $rootScope.pendingPayee;
        if (!vm.payee) {
          $location.path("/beneficiaries");
          return;
        }
        vm.transactionPassword = "";
        vm.error = "";

        vm.confirm = function () {
          vm.error = "";
          if (!vm.transactionPassword) {
            vm.error = "Please enter the TPIN.";
            return;
          }
          if (USE_MOCK_LOGIN && vm.transactionPassword !== MOCK_LOGIN.transactionPassword) {
            vm.error = "Invalid TPIN.";
            return;
          }
          const id = Math.floor(100000 + Math.random() * 900000).toString();
          $rootScope.beneficiaryStatus = {
            id,
            ...vm.payee,
          };
          $location.path("/beneficiaries/status");
        };
      },
    ]);
})();
