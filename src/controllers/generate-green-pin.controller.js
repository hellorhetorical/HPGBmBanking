(() => {
  ...
  angular.module("hpgbmbanking").controller("GenerateGreenPinController", [
      "$location",
      "$interval",
      function ($location, $interval) {
      const vm = this;
      vm.title = "Generate Green PIN";
      vm.subtitle = "Create a new PIN for your debit card.";
      vm.accounts = getMockList(MOCK_DATA.GREEN_PIN_ACCOUNTS);
      vm.form = {
      ...
      vm.getAccountLabel = function (account) {
        ...
        return `${maskedNumber} (${account.currency})`;
      };
      ...
      vm.verifyOtp = function () {
        if (vm.otpDigits.some((digit) => !digit)) {
          vm.otpError = "Please enter the complete OTP.";
          return;
        }
        const enteredOtp = vm.otpDigits.join("").trim();
        if (USE_MOCK_LOGIN && enteredOtp !== MOCK_LOGIN.otp) {
          vm.otpError = "OTP verification failed. Please try again.";
          return;
        }
        vm.otpError = "";
        vm.otpVerified = true;
      ...
      vm.submit = function () {
        if (!vm.form.newPin || vm.form.newPin !== vm.form.confirmPin) {
          vm.statusMessage = "PINs do not match. Please re-enter.";
          return;
        }
        vm.statusMessage = "";
        vm.step = "success";
